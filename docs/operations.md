...
| `OPENROUTER_API_KEY` | OpenRouter credential | Live indexing and query embedding |
...
| `index` | PostgreSQL and existing Qdrant points | Missing points and completed index alias |

Live indexing calls OpenRouter and may incur charges. Every model batch is checked for count,
...
4. Run `index`. It checks the final count and re-reads PostgreSQL before publishing the alias.
